 ...
 */
@Repository
public class OrdersDaoSQLServer implements OrdersDao
{
    ...
    public void addDemographicToPatient(Demographic demographic) throws Exception
    {
        try
        {
            jdbc.queryForObject("" + ISOLATION_READ_UNCOMMITTED
                    + "SELECT   1 "
                    + "FROM     sys.syscolumns "
                    + "WHERE    id = object_id(N'[lab21]') and sys.syscolumns.name = 'lab_demo_" + demographic.getId() + "'", Integer.class);
        } catch (EmptyResultDataAccessException ex)
        {
            jdbc.execute("ALTER TABLE lab21 ADD lab_demo_" + demographic.getId() + " " + (demographic.isEncoded() ? "INTEGER" : "VARCHAR(256)"));
        }
    }
            ...
            for (Integer year : years)
            {
                ...
                try
                {
                    jdbc.queryForObject("" + ISOLATION_READ_UNCOMMITTED
                            + "SELECT   1 "
                            + "FROM     sys.syscolumns "
                            + "WHERE    id = object_id(N'["+ lab22 +"]') and sys.syscolumns.name = 'lab_demo_" + demographic.getId() + "'", Integer.class);
                } catch (EmptyResultDataAccessException ex)
                {
                    jdbc.execute("ALTER TABLE "+ lab22 +" ADD lab_demo_" + demographic.getId() + " " + (demographic.isEncoded() ? "INTEGER" : "VARCHAR(256)"));
                }
            }
