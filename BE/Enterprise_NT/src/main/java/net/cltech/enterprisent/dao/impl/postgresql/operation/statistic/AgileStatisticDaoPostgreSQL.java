 ...
 */
@Repository
public class AgileStatisticDaoPostgreSQL implements AgileStatisticDao
{
    ...
    public List<String> getExistingYears()
    {
        try
        {
            String query = ISOLATION_READ_UNCOMMITTED + "SELECT tablename "
                    + "FROM pg_tables "
                    + "WHERE schemaname = 'public' AND tablename LIKE '%sta5%'";
