...
(function () {
  ...
  angular
    ...
    .directive('ordersearch', ordersearch);
  ordersearch.$inject = ['common', 'orderDS', 'localStorageService', '$filter'];
  /* @ngInject */
  function ordersearch(common, orderDS, localStorageService, $filter) {
    var directive = {
      templateUrl: 'app/widgets/userControl/ordersearch.html',
      restrict: 'EA',
      scope: {
        openmodal: '=openmodal',
        listener: '=listener',
      ...
      controller: ['$scope', function ($scope) {
        ...
        function searchByOrder() {
          if (vm.orderToSearch !== undefined && vm.orderToSearch !== '') {
            ...
            orderDS.getByOrder(auth.authToken, vm.orderToSearch, branch)
              .then(
                function (data) {
            ...
            document.getElementById('txt_ordersearch_order').focus();
          }
        }
        ...
        function searchByPatientId() {
          if (vm.patientIdToSearch !== undefined && vm.patientIdToSearch !== '') {
            ...
            if (vm.showDocumentType && vm.patientDocToSearch !== '0') {
              ...
              orderDS.getByPatientIdWithDoc(auth.authToken, vm.patientIdToSearch, vm.patientDocToSearch, branch)
                .then(
                  function (data) {
                    vm.loadingdata = false;
                    if (data.status === 200) {
                    ...
                    document.getElementById('txt_ordersearch_patient_id').select();
                  },
                  function (error) {
                    vm.loadingdata = false;
                    vm.Error = error;
              ...
              orderDS.getByPatientIdWithDoc(auth.authToken, vm.patientIdToSearch, 1, branch)
                .then(
                  function (data) {
                    if (data.status === 200) {
            ...
            document.getElementById('txt_ordersearch_patient_id').focus();
          }
        }
        ...
        function searchByPatient() {
          if (vm.lastNameToSearch.trim() === '' &&
            ...
            orderDS.getByPatientInfo(auth.authToken, lastNameToSearch, surNameToSearch, name1ToSearch, name2ToSearch, branch)
              .then(
                function (data) {
                  vm.loadingdata = false;
                  if (data.status === 200) {
        ...
        function searchByDate() {
          if (vm.dateToSearch !== undefined && vm.dateToSearch !== '') {
            ...
            orderDS.getByEntryDate(auth.authToken, date, branch)
              .then(
                function (data) {
                  vm.loadingdata = false;
                  if (data.status === 200) {
        ...
        function eventSelectPatient() {
          ...
          vm.name2ToSearch = '';
          setTimeout(function () {
            document.getElementById('txt_ordersearch_patient_last_name').focus();
          }, 400);
        }
