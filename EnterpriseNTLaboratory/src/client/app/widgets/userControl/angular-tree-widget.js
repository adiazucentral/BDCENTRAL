...
/* Se encontro directiva en:
    recordresultbyrank.html:
    historyassignment.html:
    queries.html:
    destinationsample.html:
    earlywarning.html:
    generalstadistics.html:
    histogram.html:
    statisticswithprices.html: */
/*
        ...
        .directive('treenode', ['RecursionHelper', function (RecursionHelper) {
            return {
                ...
                template: '<ul>'
                    ...
                    + '<span><i class="tree-node-ico" ng-if="options.showIcon"><img class="md-tree-image" ng-src="{{node.image}}"/></i>'
                    + '     <span class="node-name" ng-class="{selected: node.selected&& !node.disabled}" ng-bind-html="node.name | trust">'
                    + '         {{node.name}}'
