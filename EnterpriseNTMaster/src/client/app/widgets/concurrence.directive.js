...
(function () {
    ...
    angular
      .module('app.widgets')
      .directive('concurrence', concurrence);

    concurrence.$inject = ['$q', 'ModalService', 'testDS','configurationDS','localStorageService'];

    /* @ngInject */
    function concurrence($q, ModalService,testDS, configurationDS,localStorageService) {
        var directive = {
            templateUrl: 'app/widgets/concurrence.html',
            restrict: 'EA',
            scope: {
                disabled: '=?disabled',
                valuesConcurrence: '=valuesConcurrence'
                // ,
            ...
            controller: ['$scope', function ($scope) {
                ...
                $scope.$watch('valuesConcurrence', function () { 
                    vm.cantOpen = 0;
                    vm.cantClose = 0;
                    vm.testadd = [];
                    vm.output = '';
                    vm.outputSend = $scope.valuesConcurrence;
                    vm.testRequeridid = [];
                    vm.testRequeridabbr = [];
                    vm.textRequerid = '';
                    vm.message = false;
                ...
                function save(e) {
                    ...
                    if (vm.cantOpen !== vm.cantClose) {
                        ...
                        $scope.valuesConcurrence = vm.outputSend;
                ...
                function getTestAll() {
                    ...
                    var auth = localStorageService.get('Enterprise_NT.authorizationData');
                    return testDS.getTestArea(auth.authToken, '0').then(function (data) {
                        if (data.status === 200) {
