/*
Usage: <wysiwyg textarea-id="question" textarea-class="form-control"  textarea-height="80px" 
        textarea-name="textareaQuestion" textarea-required ng-model="question.question" 
    ...
    options
        ...
        textarea-menu           Array of Arrays that contain the groups of buttons to 
                                show Defualt:Show all button groups
...
(function(angular, undefined) {
    ...
    angular.module('app.widgets', ['colorpicker.module'])
        .directive('wysiwyg', function($timeout, wysiwgGui, $compile) {
            return {
                template: '<div>' +
                    ...
                    '   .wysiwyg-btn-group-margin { margin-right:3px; margin-bottom: 3px }' +
                    '   .wysiwyg-select { height:30px;margin-bottom:1px;}' +
                    '   .wysiwyg-colorpicker { font-family: arial, sans-serif !important;'+
                                              'font-size:16px !important; padding:2px 10px !important;}' +
                    '</style>' +
                    '<div class="wysiwyg-menu"></div>' +
                    '<div id="{{textareaId}}" ng-attr-style="resize:vertical;' +
                        'height:{{textareaHeight || \'80px\'}}; overflow:auto"' +
                        'contentEditable="{{!disabled}}" class="{{textareaClass}}'+
                        ' wysiwyg-textarea" rows="{{textareaRows}}"'+ 
                    'name="{{textareaName}}" required="{{textareaRequired}}"'+
                            ' placeholder="{{textareaPlaceholder}}" ng-model="value"></div>' +
                    '</div>',
                restrict: 'E',
                scope: {
                    value: '=ngModel',
        ...
        .factory('wysiwgGui', function($filter) {
            var ELEMENTS= {
                            'bold': {
                                tag: 'button',
