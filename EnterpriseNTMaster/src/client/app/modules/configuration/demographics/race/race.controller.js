(function () {
  'use strict';
  angular.module('app.race').controller('RaceController', RaceController);
  RaceController.$inject = ['raceDS', 'configurationDS', 'localStorageService',
    'logger', '$filter', '$state', 'moment', '$rootScope', 'LZString', '$translate'
  ];

  function RaceController(raceDS, configurationDS, localStorageService,
    logger, $filter, $state, moment, $rootScope, LZString, $translate) {
    var vm = this;
    $rootScope.menu = true;
    vm.init = init;
    vm.title = 'Race';
    vm.code = ['code', 'name', 'value', 'state'];
    vm.name = ['name', 'code', 'value', 'state'];
    vm.value = ['value', 'code', 'name', 'state'];
    vm.state = ['-state', '+code', '+name', '+value'];
    ...
    function removeData(data) {
      ...
      data.data.forEach(function (value, key) {
        value.searchall = value.code + value.name + value.value;
    ...
    function insert() {
      vm.loadingdata = true;
      var auth = localStorageService.get('Enterprise_NT.authorizationData');
      return raceDS.New(auth.authToken, vm.Detail).then(function (data) {
    ...
    function update() {
      ...
      return raceDS.update(auth.authToken, vm.Detail).then(function (data) {
        vm.loadingdata = false;
        if (data.status === 200) {
          vm.get();
          logger.success($filter('translate')('0042'));
    ...
    function get() {
      ...
      return raceDS.get(auth.authToken).then(function (data) {
        vm.loadingdata = false;
    ...
    function getId(id, index, Form) {
      ...
      vm.Detail = [];
      Form.$setUntouched();
      return raceDS.getId(auth.authToken, id).then(function (data) {
        vm.loadingdata = false;
        if (data.status === 200) {
    ...
    function generateFile() {
      if (vm.filtered.length === 0) {
        ...
        vm.pathreport = '/report/configuration/demographics/race/race.mrt';
        vm.openreport = false;
        vm.report = false;
        vm.windowOpenReport();
      }
