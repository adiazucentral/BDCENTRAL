(function () {
  ...
  paymenttypesController.$inject = ['paymenttypesDS', 'configurationDS', 'localStorageService', 'logger',
    '$filter', '$state', 'moment', '$rootScope', 'LZString', '$translate'
  ];

  function paymenttypesController(paymenttypesDS, configurationDS, localStorageService, logger,
    $filter, $state, moment, $rootScope, LZString, $translate) {
    var vm = this;
    $rootScope.menu = true;
    $rootScope.blockView = true;
    vm.init = init;
    vm.title = 'paymenttypes';
    vm.name = ['name', 'state'];
    vm.state = ['-state', '+name'];
    ...
    function insert() {
      var auth = localStorageService.get('Enterprise_NT.authorizationData');
      return paymenttypesDS.New(auth.authToken, vm.Detail).then(function (data) {
        if (data.status === 200) {
          vm.get();
          vm.Detail = data.data;
    ...
    function update() {
      ...
      return paymenttypesDS.update(auth.authToken, vm.Detail).then(function (data) {
        if (data.status === 200) {
          vm.get();
          logger.success($filter('translate')('0042'));
    ...
    function getId(id, index, Form) {
      ...
      Form.$setUntouched();
      vm.loadingdata = true;
      return paymenttypesDS.getId(auth.authToken, id).then(function (data) {
        if (data.status === 200) {
          ...
          if (data.data.id === 1) {
            data.data.name = $filter('translate')('0657');
          }
          vm.Detail = data.data;
          vm.stateButton('update');
          vm.loadingdata = false;
    ...
    function generateFile() {
      if (vm.filtered.length === 0) {
        ...
        vm.pathreport = '/report/configuration/billing/paymenttypes/paymenttypes.mrt';
        vm.openreport = false;
        vm.report = false;
        vm.windowOpenReport();
      }
