(function () {
  'use strict';
  angular.module('app.antibiotic').controller('AntibioticController', AntibioticController);
  AntibioticController.$inject = ['antibioticDS', 'configurationDS', 'localStorageService', 'logger', '$filter', '$state', 'moment', '$rootScope', 'LZString', '$translate'];

  function AntibioticController(antibioticDS, configurationDS, localStorageService, logger, $filter, $state, moment, $rootScope, LZString, $translate) {
    var vm = this;
    $rootScope.menu = true;
    ...
    function insert() {
      vm.loadingdata = true;
      var auth = localStorageService.get('Enterprise_NT.authorizationData');
      return antibioticDS.New(auth.authToken, vm.Detail).then(function (data) {
        if (data.status === 200) {
          vm.get();
          vm.Detail = data.data;
    ...
    function update() {
      ...
      return antibioticDS.update(auth.authToken, vm.Detail).then(function (data) {
        if (data.status === 200) {
          vm.get();
          logger.success($filter('translate')('0042'));
    ...
    function get() {
      ...
      return antibioticDS.get(auth.authToken).then(function (data) {
    ...
    function getId(id, index, Form) {
      ...
      Form.$setUntouched();
      vm.loadingdata = true;
      return antibioticDS.getId(auth.authToken, id).then(function (data) {
        if (data.status === 200) {
    ...
    function generateFile() {
      if (vm.filtered.length === 0) {
        ...
        vm.pathreport = '/report/configuration/microbiology/antibiotic/antibiotic.mrt';
        vm.openreport = false;
        vm.report = false;
        vm.windowOpenReport();
      }
