(function () {
  ...
  angular
    ...
    .controller('LiteralResultController', LiteralResultController);

  LiteralResultController.$inject = ['literalresultDS', 'configurationDS', 'localStorageService', 'logger',
    '$filter', '$state', 'moment', '$rootScope', 'LZString', '$translate'
  ];

  function LiteralResultController(literalresultDS, configurationDS, localStorageService, logger,
    $filter, $state, moment, $rootScope, LZString, $translate) {
    ...
    function insert() {
      var auth = localStorageService.get('Enterprise_NT.authorizationData');
      return literalresultDS.New(auth.authToken, vm.Detail).then(function (data) {
        if (data.status === 200) {
          vm.get();
          vm.Detail = data.data;
    ...
    function update() {
      ...
      return literalresultDS.update(auth.authToken, vm.Detail).then(function (data) {
        if (data.status === 200) {
          vm.get();
          logger.success($filter('translate')('0042'));
    ...
    function get() {
      ...
      return literalresultDS.get(auth.authToken).then(function (data) {
    ...
    function getId(id, index, Form) {
      ...
      Form.$setUntouched();
      vm.loadingdata = true;
      return literalresultDS.getId(auth.authToken, id).then(function (data) {
        if (data.status === 200) {
    ...
    function generateFile() {
      if (vm.filtered.length === 0) {
        ...
        vm.pathreport = '/report/configuration/test/literalresult/literalresult.mrt';
        vm.openreport = false;
        vm.report = false;
        vm.windowOpenReport();
      }
