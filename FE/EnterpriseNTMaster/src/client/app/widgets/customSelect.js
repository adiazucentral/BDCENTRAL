			...
			link: function (scope, elem, attrs, controller) {
					...
					'<div class="dropdown-menu">' +
						...
						'<ul role="menu">' +
							...
							'<li ng-hide="matches.length" class="empty-result" stop-propagation="click">' +
								'<em class="muted">' +
									'<span ng-hide="searchTerm">{{"0464" | translate}}</span>' +
									'<span class="word-break" ng-show="searchTerm">{{"0465" | translate}}</span>' +
								'</em>' +
							'</li>' +
						'</ul>' +
				...
				function setDisplayText() {
					...
					/*if (text === undefined || text === ' ' ) {
						...
						//if (map) {
							text = $filter('translate')('0463')
						//}
					}*/

					if (childScope.matches !== undefined) {
					if (controller.$modelValue === undefined ) {
						...
						for (var i = 0; i < listElement.length; i++) {
							if (listElement[i].value === controller.$modelValue ) {
								text = listElement[i].label;
								//break; 
							}
						}
					}
