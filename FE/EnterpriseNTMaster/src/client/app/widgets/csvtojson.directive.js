...
(function () {
    ...
    angular
      .module('app.widgets')
      .directive('csvtojson', csvtojson);

    csvtojson.$inject = ['$q','ModalService','localStorageService','configurationDS'];

    /* @ngInject */
    function csvtojson($q,ModalService,localStorageService,configurationDS) {
        var directive = {
            templateUrl: 'app/widgets/csvtojson.html',
            restrict: 'EA',
            scope: {
                disabled: '=?disabled',
                json: '=?',
            ...
            controller: ['$scope', function ($scope) {
                ...
                vm.numberbutton = $scope.numberbutton;
                ...
                $scope.$watch('errorarray', function () {  
                    ...
                    if (vm.errorarray !== undefined) {
                        ModalService.showModal({
                            templateUrl: 'validationFormatcsv.html',
                            controller: 'validationFormatcsvController',
                            inputs: {
            ...
            controllerAs: 'csvtojson'
        };
        return directive;
    }
    angular 
    .module('app.widgets')
    .controller('validationFormatcsvController', validationFormatcsvController);
    validationFormatcsvController.$inject = ['$scope','close','correctImport', 'quantityOrigin', 'quantitydata', 'errorarray', 'arraydata','$filter'];

    function validationFormatcsvController($scope, close, correctImport, quantityOrigin, quantitydata, errorarray, arraydata,$filter) {
