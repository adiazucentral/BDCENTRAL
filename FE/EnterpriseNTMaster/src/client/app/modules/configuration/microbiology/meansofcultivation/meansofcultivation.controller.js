(function () {
  ...
  angular
    ...
    .controller('MeansofcultivationController', MeansofcultivationController);

  MeansofcultivationController.$inject = ['mediaCulturesDS', 'configurationDS', '$http', 'localStorageService', 'logger',
    'authService', '$filter', '$state', 'moment', '$rootScope', 'LZString', '$translate'
  ];

  function MeansofcultivationController(mediaCulturesDS, configurationDS, $http, localStorageService, logger, authService,
    ...
    function insert() {
      vm.loadingdata = true;
      var auth = localStorageService.get('Enterprise_NT.authorizationData');
      return mediaCulturesDS.New(auth.authToken, vm.Detail).then(function (data) {
        if (data.status === 200) {
          vm.get();
          vm.Detail = data.data;
    ...
    function update() {
      ...
      return mediaCulturesDS.update(auth.authToken, vm.Detail).then(function (data) {
        if (data.status === 200) {
          vm.get();
          logger.success($filter('translate')('0042'));
    ...
    function get() {
      ...
      return mediaCulturesDS.get(auth.authToken).then(function (data) {
    ...
    function getId(id, index, Form) {
      ...
      vm.Detail = [];
      Form.$setUntouched();
      return mediaCulturesDS.getId(auth.authToken, id).then(function (data) {
        if (data.status === 200) {
    ...
    function generateFile() {
      if (vm.filtered.length === 0) {
        ...
        vm.pathreport = '/report/configuration/microbiology/meansofcultivation/meansofcultivation.mrt';
        vm.openreport = false;
        vm.report = false;
        vm.windowOpenReport();
      }
