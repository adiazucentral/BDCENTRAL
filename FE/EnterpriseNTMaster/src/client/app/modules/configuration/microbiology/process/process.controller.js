(function () {
  ...
  ProcessController.$inject = ['processDS', 'configurationDS', 'localStorageService', 'logger',
    '$filter', '$state', 'moment', '$rootScope', 'LZString', '$translate'
  ];

  function ProcessController(processDS, configurationDS, localStorageService, logger,
    $filter, $state, moment, $rootScope, LZString, $translate) {
    ...
    function insert() {
      vm.loadingdata = true;
      var auth = localStorageService.get('Enterprise_NT.authorizationData');
      return processDS.New(auth.authToken, vm.Detail).then(function (data) {
        if (data.status === 200) {
          vm.get();
          vm.Detail = data.data;
    ...
    function update() {
      ...
      return processDS.update(auth.authToken, vm.Detail).then(function (data) {
        if (data.status === 200) {
          vm.get();
          logger.success($filter('translate')('0042'));
    ...
    function get() {
      ...
      return processDS.get(auth.authToken).then(function (data) {
    ...
    function getId(id, index, Form) {
      ...
      vm.Detail = [];
      Form.$setUntouched();
      return processDS.getId(auth.authToken, id).then(function (data) {
        if (data.status === 200) {
    ...
    function generateFile() {
      if (vm.filtered.length === 0) {
        ...
        vm.pathreport = '/report/configuration/microbiology/process/process.mrt';
        vm.openreport = false;
        vm.report = false;
        vm.windowOpenReport();
      }
