(function () {
  'use strict';
  angular.module('app.destination').controller('DestinationController', DestinationController);
  DestinationController.$inject = ['destinationDS', 'configurationDS', 'listDS', 'localStorageService',
    'logger', '$filter', '$state', 'moment', '$rootScope', 'LZString', '$translate'
  ];

  function DestinationController(destinationDS, configurationDS, listDS, localStorageService,
    logger, $filter, $state, moment, $rootScope, LZString, $translate) {
    var vm = this;
    $rootScope.menu = true;
    $rootScope.blockView = true;
    vm.init = init;
    vm.title = 'Destination';
    vm.code = ['code', 'name', 'typename', 'color', 'state'];
    vm.name = ['name', 'code', 'typename', 'color', 'state'];
    vm.typename = ['typename', 'code', 'name', 'color', 'state'];
    vm.color = ['color', 'code', 'name', 'typename', 'state'];
    vm.state = ['-state', '+code', '+name', '+typename', '+color'];
    ...
    function getListType() {
      var auth = localStorageService.get('Enterprise_NT.authorizationData');
      return listDS.getList(auth.authToken, 43).then(function (data) {
        var typemotive = [];
        if (($filter('translate')('0000')) === 'esCo') {
          data.data.forEach(function (value, key) {
            var object = {
        ...
        vm.Listype = $filter('orderBy')(typemotive, 'name');
        vm.get();
      }, function (error) {
        vm.modalError(error);
      });
    ...
    function insert() {
      vm.loadingdata = true;
      var auth = localStorageService.get('Enterprise_NT.authorizationData');
      return destinationDS.New(auth.authToken, vm.Detail).then(function (data) {
        if (data.status === 200) {
          vm.get();
          vm.Detail = data.data;
    ...
    function update() {
      ...
      return destinationDS.update(auth.authToken, vm.Detail).then(function (data) {
        if (data.status === 200) {
          vm.get();
          logger.success($filter('translate')('0042'));
    ...
    function get() {
      ...
      return destinationDS.get(auth.authToken).then(function (data) {
    ...
    function getId(id, index, Form) {
      ...
      Form.$setUntouched();
      vm.loadingdata = true;
      return destinationDS.getId(auth.authToken, id).then(function (data) {
        if (data.status === 200) {
    ...
    function generateFile() {
      if (vm.filtered.length === 0) {
        ...
        vm.pathreport = '/report/configuration/traceability/destination/destination.mrt';
        vm.openreport = false;
        vm.report = false;
        vm.windowOpenReport();
      }
