(function () {
  'use strict';
  angular.module('app.motive').controller('MotiveController', MotiveController);
  MotiveController.$inject = ['motiveDS', 'configurationDS', 'listDS', 'localStorageService',
    'logger', '$filter', '$state', 'moment', '$rootScope', 'LZString', '$translate'
  ];

  function MotiveController(motiveDS, configurationDS, listDS, localStorageService,
    logger, $filter, $state, moment, $rootScope, LZString, $translate) {
    var vm = this;
    $rootScope.menu = true;
    ...
    vm.init = init;
    vm.title = 'Motive';
    vm.name = ['name', 'typename', 'state'];
    vm.typename = ['typename', 'name', 'state'];
    vm.state = ['-state', '+name', '+typename'];
    ...
    function getListType() {
      var auth = localStorageService.get('Enterprise_NT.authorizationData');
      return listDS.getList(auth.authToken, 14).then(function (data) {
        var typemotive = [];
        if (($filter('translate')('0000')) === 'esCo') {
          data.data.forEach(function (value, key) {
            var object = {
    ...
    function insert() {
      vm.loadingdata = true;
      var auth = localStorageService.get('Enterprise_NT.authorizationData');
      return motiveDS.New(auth.authToken, vm.Detail).then(function (data) {
        if (data.status === 200) {
          vm.get();
          vm.Detail = data.data;
    ...
    function update() {
      ...
      return motiveDS.update(auth.authToken, vm.Detail).then(function (data) {
        if (data.status === 200) {
          vm.get();
          logger.success($filter('translate')('0042'));
    ...
    function get() {
      ...
      return motiveDS.get(auth.authToken).then(function (data) {
    ...
    function getId(id, index, Form) {
      ...
      vm.Detail = [];
      Form.$setUntouched();
      return motiveDS.getId(auth.authToken, id).then(function (data) {
        if (data.status === 200) {
    ...
    function generateFile() {
      if (vm.filtered.length === 0) {
        ...
        vm.pathreport = '/report/configuration/traceability/motive/motive.mrt';
        vm.openreport = false;
        vm.report = false;
        vm.windowOpenReport();
      }
