(function () {
  ...
  documenttypesController.$inject = ['documenttypesDS', 'configurationDS', 'localStorageService', 'logger',
    '$filter', '$state', 'moment', '$rootScope', 'LZString', '$translate'
  ];

  function documenttypesController(documenttypesDS, configurationDS, localStorageService, logger,
    $filter, $state, moment, $rootScope, LZString, $translate) {
    var vm = this;
    $rootScope.menu = true;
    $rootScope.blockView = true;
    vm.init = init;
    vm.title = 'Document Types';
    ...
    {
      id: 1,
      name: $filter('translate')('1187')
    },
    {
      id: 2,
      name: $filter('translate')('1188')
    }, 
    {
      id: 3,
      name: $filter('translate')('1189')
    },
    {
      id: 4,
      name: $filter('translate')('1190')
    },
    {
      id: 5,
      name: $filter('translate')('1191')
    },
    {
      id: 6,
      name: $filter('translate')('1192')
    }
  ];
    ...
    function removeData(data) {
      ...
      data.data.forEach(function (value, key) {
        value.searchall = value.abbr + value.name + value.value;
    ...
    function insert() {
      vm.loadingdata = true;
      var auth = localStorageService.get('Enterprise_NT.authorizationData');
      return documenttypesDS.New(auth.authToken, vm.Detail).then(function (data) {
    ...
    function update() {
      ...
      vm.loadingdata = true;
      return documenttypesDS.update(auth.authToken, vm.Detail).then(function (data) {
        vm.loadingdata = false;
        if (data.status === 200) {
          vm.get();
          logger.success($filter('translate')('0042'));
    ...
    function modalError(error) {
      ...
      if (vm.Repeatabbr === false && vm.Repeatname === false) {
        vm.Error = error;
    ...
    function get() {
      ...
      return documenttypesDS.get(auth.authToken).then(function (data) {
        vm.loadingdata = false;
    ...
    function getId(id, index, Form) {
      ...
      vm.Detail = [];
      Form.$setUntouched();
      return documenttypesDS.getId(auth.authToken, id).then(function (data) {
        vm.loadingdata = false;
        if (data.status === 200) {
    ...
    function generateFile() {
      if (vm.filtered.length === 0) {
        ...
        vm.pathreport = '/report/configuration/demographics/documenttype/documenttype.mrt';
        vm.openreport = false;
        vm.report = false;
        vm.windowOpenReport();
      }
