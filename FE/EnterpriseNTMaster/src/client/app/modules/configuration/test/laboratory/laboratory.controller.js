(function () {
  ...
  LaboratoryController.$inject = ['laboratoryDS', 'configurationDS', 'localStorageService', 'logger',
    '$filter', '$state', 'moment', '$rootScope', 'LZString', '$translate'
  ];

  function LaboratoryController(laboratoryDS, configurationDS, localStorageService, logger,
    $filter, $state, moment, $rootScope, LZString, $translate) {
    var vm = this;
    $rootScope.menu = true;
    vm.init = init;
    ...
    function removeData(data) {
      ...
      data.data.forEach(function (value, key) {
        ...
        if (value.type === 1) {
          value.type = $filter('translate')('0215');
        }
        if (value.type === 2) {
          value.type = $filter('translate')('0216');
        }
        data.data[key].username = auth.userName;
        data.data[key].alldata = value.code + value.name + value.type;
      });
      return data.data;
    }
    ...
    function insert() {
      var auth = localStorageService.get('Enterprise_NT.authorizationData');
      return laboratoryDS.New(auth.authToken, vm.Detail).then(function (data) {
    ...
    function update() {
      ...
      return laboratoryDS.update(auth.authToken, vm.Detail).then(function (data) {
        vm.loadingdata = false;
        if (data.status === 200) {
          vm.get();
          logger.success($filter('translate')('0042'));
    ...
    function get() {
      ...
      return laboratoryDS.get(auth.authToken).then(function (data) {
        vm.loadingdata = false;
    ...
    function generateFile() {
      if (vm.filtered.length === 0) {
        ...
        vm.pathreport = '/report/configuration/test/laboratory/laboratory.mrt';
        vm.openreport = false;
        vm.report = false;
        vm.windowOpenReport();
      }
