...
(function () {
  ...
  angular
    .module('app.widgets')
    .directive('modalpostponement', modalpostponement);

  modalpostponement.$inject = ['motiveDS', 'sampletrackingsDS', 'localStorageService', '$filter', 'patientDS', 'common', '$rootScope', 'logger'];
  /* @ngInject */
  function modalpostponement(motiveDS, sampletrackingsDS, localStorageService, $filter, patientDS, common, $rootScope, logger) {
    var directive = {
      templateUrl: 'app/widgets/userControl/modal-postponement.html',
      restrict: 'EA',
      scope: {
        openmodal: '=openmodal',
      ...
      controller: ['$scope', function ($scope) {
        ...
        function getestinit(idSample, active) {
          ...
          if (vm.listTest.length === 0) {
                ...
                if (data.status === 200) {
                  ...
                  vm.Detail.tests.forEach(function (value, key) {
                    if (value.testType !== 0) {
                      ...
                      value.ordersort = (value.id + 101).toString();
                      ...
                      vm.Detail.tests[key].profileId = value.profile;
                      ...
                      vm.Detail.tests[key].ordersort = (value.profile + 101).toString();
                      vm.Detail.tests[key].select = false;
                  ...
                  vm.Detail.tests = _.orderBy(vm.Detail.tests, 'ordersort', 'asc');
            ...
            /*     vm.listSamples = _.filter(vm.listTest, function (o) { return o.sampleState !== 4; });
                if (vm.listSamples.length === 0) {
                  vm.loadingdata = false;
                  vm.loading = false;
                  logger.info("Las muestras no han sido ingresadas al laboratorio por favor confirme su estado");
                } else { */
            vm.samples = [];
            vm.active = active;
            if (active) {
